...
extern "C" {
  ...
  typedef struct context {
    ...
    Realm::ProcessorGroup procgroup;
    size_t numprocs;
    unsigned cur_task;
  } context;
  ...
  int realmInitRuntime(int argc, char** argv) {
    ...
    Realm::Machine::ProcessorQuery procquery(Realm::Machine::get_machine()).only_kind(Realm::Processor::LOC_PROC); 

    _globalCTX->numprocs = procquery.count();
    ...
    for(auto it = procquery.begin(); it != procquery.end(); it++)
