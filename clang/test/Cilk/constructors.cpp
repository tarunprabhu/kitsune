...
void rule_of_four() {
  ...
  // CHECK: [[INVOKECONT]]:
  // CHECK-NEXT: %[[CALL:.+]] = invoke {{.*}}dereferenceable(16) %class.Bar* @_ZN3BaraSES_(ptr {{.*}}dereferenceable(16) %[[b0:.+]], ptr {{.*}}%[[AGGTMP]])
...
void derived_class() {
  ...
  // CHECK: invoke void @_Z15makeDBarFromBar3Bar(ptr {{.*}}sret(%class.DBar) {{.*}}%[[REFTMP2]], %class.Bar* {{.*}}%[[AGGTMP]])
  ...
  // CHECK: invoke void @_Z15makeDBarFromBar3Bar(ptr {{.*}}sret(%class.DBar) {{.*}}%[[REFTMP3]], %class.Bar* {{.*}}%[[AGGTMP2]])
