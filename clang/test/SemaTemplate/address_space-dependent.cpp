...
template <long int I>
void tooBig() {
  __attribute__((address_space(I))) int *bounds; // expected-error {{address space is larger than the maximum supported (4194281)}}
}
...
int main() {
  ...
  // The original here was 0x7FFFFE9. But Kitsune has stolen a bit from the
  // address space to accommodate the mobile bit, so we only have 22 bits now.
  correct<0x3FFFE9>();
