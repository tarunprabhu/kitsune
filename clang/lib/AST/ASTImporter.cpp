...
template <>
LLVM_NODISCARD Error ASTImporter::importInto(SourceLocation &To,
...
// FIXME: Temporary until every import returns Expected.
template <>
LLVM_NODISCARD Error ASTImporter::importInto(QualType &To,
                                             const QualType &From) {
  To = Import(From);
...
ASTNodeImporter::VisitConstantArrayType(const ConstantArrayType *T) {
  ...
  return Importer.getToContext().getConstantArrayType(
      *ToElementTypeOrErr, T->getSize(), T->getSizeModifier(),
...
ASTNodeImporter::ImportTemplateParameterList(TemplateParameterList *Params) {
  ...
  return TemplateParameterList::Create(
      Importer.getToContext(), *ToTemplateLocOrErr, *ToLAngleLocOrErr, ToParams,
      *ToRAngleLocOrErr, ToRequiresClause);
}
...
ExpectedDecl ASTNodeImporter::VisitFunctionDecl(FunctionDecl *D) {
  ...
  NestedNameSpecifierLoc ToQualifierLoc;
  if (auto Imp =
          importSeq(FromTy, D->getTypeSourceInfo(), D->getInnerLocStart(),
  ...
  if (auto *FromConstructor = dyn_cast<CXXConstructorDecl>(D)) {
    if (GetImportedOrCreateDecl<CXXConstructorDecl>(
            ...
            ToInnerLocStart, NameInfo, T, TInfo, FromConstructor->isExplicit(),
...
ExpectedDecl ASTNodeImporter::VisitObjCMethodDecl(ObjCMethodDecl *D) {
  ...
  if (GetImportedOrCreateDecl(
          ...
          D->isImplicit(), D->isDefined(), D->getImplementationControl(),
...
ExpectedDecl ASTNodeImporter::VisitVarTemplateSpecializationDecl(
  ...
  if (D2) {
    ...
    if (auto *FromPartial = dyn_cast<PartVarSpecDecl>(D)) {
      ...
      auto ToTPListOrErr =
          ImportTemplateParameterList(FromPartial->getTemplateParameters());
      if (!ToTPListOrErr)
        return ToTPListOrErr.takeError();
...
ExpectedStmt ASTNodeImporter::VisitGCCAsmStmt(GCCAsmStmt *S) {
  ...
  return new (Importer.getToContext())
      GCCAsmStmt(Importer.getToContext(), *AsmLocOrErr, S->isSimple(),
                 S->isVolatile(), S->getNumOutputs(), S->getNumInputs(),
                 Names.data(), Constraints.data(), Exprs.data(), *AsmStrOrErr,
                 S->getNumClobbers(), Clobbers.data(), *RParenLocOrErr);
}
...
ExpectedStmt ASTNodeImporter::VisitForallStmt(ForallStmt *S) {
  ...
  return new (Importer.getToContext())
      ForallStmt(Importer.getToContext(), ToInit, ToCond, ToConditionVariable,
                 ToInc, ToBody, ToForLoc, ToLParenLoc, ToRParenLoc);
}
...
ASTNodeImporter::VisitMaterializeTemporaryExpr(MaterializeTemporaryExpr *E) {
  auto Imp =
      importSeq(E->getType(), E->GetTemporaryExpr(), E->getExtendingDecl());
  if (!Imp)
    return Imp.takeError();
...
ExpectedStmt ASTNodeImporter::VisitCXXDependentScopeMemberExpr(
  ...
  auto Imp =
      importSeq(E->getType(), E->getOperatorLoc(), E->getQualifierLoc(),
...
ASTNodeImporter::VisitDependentScopeDeclRefExpr(DependentScopeDeclRefExpr *E) {
  auto Imp = importSeq(E->getQualifierLoc(), E->getTemplateKeywordLoc(),
                       E->getDeclName(), E->getExprLoc(), E->getLAngleLoc(),
                       E->getRAngleLoc());
  if (!Imp)
    return Imp.takeError();
  ...
  DeclarationName ToDeclName;
  std::tie(ToQualifierLoc, ToTemplateKeywordLoc, ToDeclName, ToExprLoc,
...
ASTNodeImporter::VisitUnresolvedMemberExpr(UnresolvedMemberExpr *E) {
  auto Imp1 = importSeq(E->getType(), E->getOperatorLoc(), E->getQualifierLoc(),
...
NestedNameSpecifier *ASTImporter::Import(NestedNameSpecifier *FromNNS) {
  ...
  case NestedNameSpecifier::NamespaceAlias:
    if (auto *NSAD = cast_or_null<NamespaceAliasDecl>(
            Import(FromNNS->getAsNamespaceAlias()))) {
...
NestedNameSpecifierLoc ASTImporter::Import(NestedNameSpecifierLoc FromNNS) {
  ...
  while (!NestedNames.empty()) {
    ...
    switch (Kind) {
    case NestedNameSpecifier::Identifier:
      Builder.Extend(getToContext(), Spec->getAsIdentifier(),
                     Import(NNS.getLocalBeginLoc()),
                     Import(NNS.getLocalEndLoc()));
      break;

    case NestedNameSpecifier::Namespace:
      Builder.Extend(getToContext(), Spec->getAsNamespace(),
                     Import(NNS.getLocalBeginLoc()),
                     Import(NNS.getLocalEndLoc()));
      break;
    ...
    case NestedNameSpecifier::TypeSpecWithTemplate: {
      ...
      Builder.Extend(getToContext(), Import(NNS.getLocalBeginLoc()),
                     TSI->getTypeLoc(), Import(NNS.getLocalEndLoc()));
      break;
    }
    ...
    case NestedNameSpecifier::Super: {
      ...
      Builder.MakeSuper(getToContext(), Spec->getAsRecordDecl(),
                        ToRange.getBegin(), ToRange.getEnd());
    }
    }
  }
...
TemplateName ASTImporter::Import(TemplateName From) {
  ...
  case TemplateName::SubstTemplateTemplateParmPack: {
    ...
    auto *Param = cast_or_null<TemplateTemplateParmDecl>(
...
CXXCtorInitializer *ASTImporter::Import(CXXCtorInitializer *From) {
  ...
  if (From->isBaseInitializer()) {
    ...
    auto *ToIField =
        cast_or_null<IndirectFieldDecl>(Import(From->getIndirectMember()));
  ...
  if (auto *FromDC = cast<DeclContext>(From)) {
    ...
    if (auto *ToIFace = dyn_cast<ObjCInterfaceDecl>(To)) {
      if (!ToIFace->getDefinition()) {
        return Importer.ImportDefinition(cast<ObjCInterfaceDecl>(FromDC),
                                         ToIFace,
                                         ASTNodeImporter::IDK_Everything);
      }
    }
    ...
    if (auto *ToProto = dyn_cast<ObjCProtocolDecl>(To)) {
      if (!ToProto->getDefinition()) {
        return Importer.ImportDefinition(cast<ObjCProtocolDecl>(FromDC),
                                         ToProto,
                                         ASTNodeImporter::IDK_Everything);
      }
    }
