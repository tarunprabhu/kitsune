...
template <typename Derived>
StmtResult TreeTransform<Derived>::TransformSpawnStmt(SpawnStmt *S) {
  ...
  return getDerived().RebuildSpawnStmt(S->getSpawnLoc(), S->getSyncVar(),
                                       Child.get());
}

template <typename Derived>
StmtResult TreeTransform<Derived>::TransformSyncStmt(SyncStmt *S) {
  return S;
}
