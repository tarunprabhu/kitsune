...
template <typename CXXForLikeStmt>
static void DiagnoseForRangeVariableCopies(Sema &SemaRef,
                                           const CXXForLikeStmt *ForStmt) {
...
StmtResult Sema::FinishCXXForallRangeStmt(Stmt *S, Stmt *B) {
  ...
  // We can just use the checks for the regular for range because for now, we
  // should have the same constraints.
  // KITSUNE FIXME: Do we really have the same constraints? We can't codegen
  // an arbitrary range-based forall anyway. We should reconsider supporting
  // this at all. If we do, we should find some way of asserting that it is
  // only relevant for very limited cases.
  DiagnoseForRangeVariableCopies(*this, ForStmt);
...
StmtResult Sema::ActOnForallStmt(SourceLocation ForLoc,
  ...
  // cases. Writing to a variable outside of the forall will cause the code to
  ...
  //
  // In general, we do not allow spawn's inside forall statements at all because
  // there are certain combinations of tapir targets for which this cannot
  // work in any reasonable way. For instance, if a forall is codegen'ed for an
  // NVIDIA GPU, a kernel will be created for the loop which will run on said
  // GPU. At the time of writing, such a kernel cannot spawn extra work - either
  // in the form of another kernel call or as threads of execution. This is
  // even more complicated because the check needs to be transitive -- any
  // functions that may be called from a forall body must not contain a spawn.
  // It is not at all clear how we can enforce this.
  //
  // For now, we just carry out a very naive, top-level check. We really ought
