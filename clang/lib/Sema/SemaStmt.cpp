...
namespace {
  ...
  public:
    ...
    void VisitKitsuneForallStmt(const KitsuneForallStmt *S) {
      // Only visit the init statement of a for loop; the body
      // has a different break/continue scope.
      if (const Stmt *Init = S->getInit())
        Visit(Init);
    ...
    void VisitKitsuneForallRangeStmt(const KitsuneForallRangeStmt *S) {
      // Only visit the initialization of a for loop; the body
      // has a different break/continue scope.
      if (const Stmt *Init = S->getInit())
        Visit(Init);
...
StmtResult Sema::ActOnForStmt(SourceLocation ForLoc, SourceLocation LParenLoc,
  ...
  if (Body->getStmtClass() == Stmt::KitsuneSpawnStmtClass)
...
/// If the attempt fails, this function will return a valid, null StmtResult
/// and emit no diagnostics.
static StmtResult RebuildKitsuneForallRangeWithDereference(Sema &SemaRef, Scope *S,
                                                 SourceLocation ForLoc,
                                                 SourceLocation CoawaitLoc,
                                                 Stmt *InitStmt,
                                                 Stmt *LoopVarDecl,
...
/// BuildKitsuneForallRangeStmt - Build or instantiate a C++11 for-range 
/// statement where each iteration of the loop can be executed independently
/// of the others.
// FIXME: Kitsune: This is essentially a clone of the code for the regular
// C++11 for-range statement. We may be able to juse use the code for C++11
// for range and augment it to actually check the parallel semantics.
// TODO: We may want to consider if we even want to support this mode of
// execution. It will only work with certain backends, and if the goal is to
// take advantage of Tapir's fork-join semantics and just fork each iteration,
// and have it run concurrently with the others, there might be a better way
// of expressing that behavior in the language.
StmtResult Sema::BuildKitsuneForallRangeStmt(SourceLocation ForLoc, 
                                         SourceLocation CoawaitLoc, 
                                         Stmt *InitStmt,
                                         SourceLocation ColonLoc, 
                                         Stmt *RangeDecl, 
...
/// 3) for (const foo x : foos) where x is constructed from a reference foo.
///    Suggest "const foo &x" to prevent the copy.
// FIXME: Kitsune: This is basically a clone of the code for the For statement
// above. It might be better to just use that code instead and augment it with 
// any additional checks that we need since we have additional semantics.
static void DiagnoseForallRangeVariableCopies(Sema &SemaRef,
                                              const KitsuneForallRangeStmt *ForStmt) {
  if (SemaRef.inTemplateInstantiation())
    return;
...
/// This is a separate step from ActOnKitsuneForallRangeStmt because analysis
/// of the body cannot be performed until after the type of the range variable
/// is determined.
// FIXME: Kitsune: This is a copy of FinishForRangeStmt. It may be better to
// use that code instead and augment it with anything that we particularly 
// need because of the parallel semantics of our forall statement.
StmtResult Sema::FinishKitsuneForallRangeStmt(Stmt *S, Stmt *B) {
  if (!S || !B)
    return StmtError();
...
// FIXME: Kitsune: How similar is this to the regular For Stmt code? It may
// be better to just use that instead.
StmtResult Sema::ActOnKitsuneForallStmt(SourceLocation ForLoc, SourceLocation LParenLoc,
                              Stmt *First, ConditionResult Second,
                              FullExprArg third, SourceLocation RParenLoc,
                              Stmt *Body) {
  if (Second.isInvalid())
...
// FIXME: Kitsune: How similar is this to the range-based For statement.
// Could we just use that code instead?
StmtResult Sema::ActOnKitsuneForallRangeStmt(Scope *S, SourceLocation ForLoc,
                                      SourceLocation CoawaitLoc, Stmt *InitStmt,
                                      Stmt *First, SourceLocation ColonLoc,
                                      Expr *Range, SourceLocation RParenLoc,
                                      BuildForRangeKind Kind) {
