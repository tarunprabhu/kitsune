...
static void DiagUninitUse(Sema &S, const VarDecl *VD, const UninitUse &Use,
  ...
  for (UninitUse::branch_iterator I = Use.branch_begin(), E = Use.branch_end();
    ...
    case Stmt::CXXForallRangeStmtClass:
      if (I->Output == 1) {
        // The use occurs if a range-based forall loop's body never executes.
        // That may be impossible, and there's no syntactic fix for this,
        // so treat it as a 'may be uninitialized' case.
        continue;
      }
      DiagKind = 1;
      Str = "forall";
      Range = cast<CXXForallRangeStmt>(Term)->getRangeInit()->getSourceRange();
      break;
...
public:
  ...
  void handleDoubleLock(StringRef Kind, Name LockName,
                        SourceLocation Loc) override {
