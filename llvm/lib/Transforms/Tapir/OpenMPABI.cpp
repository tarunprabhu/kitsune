...
bool OpenMPABI::preProcessFunction(Function &F, TaskInfo &TI,
  ...
  // FIXME: This should only return true if the CFG was modified. If neither
  // of the functions called above modify the CFG, this should be changed.
  llvm_unreachable("Deliberately crashing because a human needs to check the return value");
  return true;
}
