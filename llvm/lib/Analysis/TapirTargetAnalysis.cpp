...
void TapirTargetInfo::computeRequiredTTs(Module &m, GetLoopInfo getLoopInfo,
  ...
  std::set<TapirTargetID> ttsForModule;
  for (Function &f : m.functions()) {
    ...
    std::set<TapirTargetID> ttsForFunc;
    for (const Loop *tl : li) {
      for (const Loop *loop : post_order(tl)) {
        if (getTaskIfTapirLoopStructure(loop, &ti)) {
          TapirTargetID tt = ttOpts->getID();
          if (std::optional<TapirTargetID> hintTT =
                  TapirLoopHints(loop).getLoopTarget())
            tt = *hintTT;
...
TapirTargetID TapirTargetInfo::getID() const {
  assert(ttOpts && "Tapir target options have not been set");
  return ttOpts->getID();
}

std::optional<TapirTargetID> TapirTargetInfo::getIDIfExists() const {
  if (ttOpts)
    return ttOpts->getID();
  return std::nullopt;
}
...
const std::vector<TapirTargetID> &
TapirTargetInfo::getRequiredTTs(Function &f) const {
...
const std::vector<TapirTargetID> &
TapirTargetInfo::getRequiredTTs(Module &) const {
  return ttsInModule;
}
