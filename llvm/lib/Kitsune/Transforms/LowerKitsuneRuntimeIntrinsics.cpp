//===- LowerKitsuneRuntimeIntrinsics.cpp - Lower kitrt intrinsics ---------===//
//
...
private:
  ...
  TTID getTTID(CallBase &call) {
    if (auto *cint = dyn_cast<ConstantInt>(call.getArgOperand(0)))
      return TTID(cint->getZExtValue());
  ...
  FunctionCallee getKitsuneRuntimeFunc(CallBase &call) {
    ...
    case Intrinsic::kitrt_hip_enable_xnack: {
      const KitsuneRuntimeFuncMap &funcs = kitTTFuncs.at(TTID::Hip);
