...
private:
  ...
  MaybeGlobalsBCFB getGlobalsBCFB(Module &m, TTID tt) const {
    GlobalsBCFB res;
    for (GlobalVariable &g : m.globals()) {
      if (getAttrValueAsTTID(g, Attribute::KitBC) == tt)
        res.bc = &g;
      else if (getAttrValueAsTTID(g, Attribute::KitFB) == tt)
        res.fb = &g;
    }
...
public:
  ...
  bool run(Module &m) {
    ...
    if (MaybeGlobalsBCFB r = getGlobalsBCFB(m, TTID::Cuda)) {
      detail::cgFatBinaryCuda(*r->fb, *r->bc, ttOpts, clKeepFiles);
      r->bc->eraseFromParent();
      changed |= true;
    }

    if (MaybeGlobalsBCFB r = getGlobalsBCFB(m, TTID::Hip)) {
