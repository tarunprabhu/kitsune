...
ModulePassManager
PassBuilder::buildKitsunePreTapirPipeline(OptimizationLevel Level,
                                          ThinOrFullLTOPhase Phase) {
  ModulePassManager MPM;
  ...
  // which probably also need to be removed. This should not be done in the LTO
  // prelink phase because the intrinsics may be needed by Kitsune's lowering
  // passes which will be run only at full LTO time.
  if (!isLTOPreLink(Phase)) {
...
ModulePassManager
PassBuilder::buildKitsunePostTapirPipeline(OptimizationLevel Level,
                                           ThinOrFullLTOPhase Phase) {
  ModulePassManager MPM;

  // Tapir lowering will have run only if the TapirTargetOptions have been set,
  // if the target is one that must be lowered and this is not an LTO prelink
  // phase.
  if (PTO.TTOpts && PTO.TTOpts->lower() && !isLTOPreLink(Phase)) {
    ...
    // Currently, there are no passes that should be run if tapir lowering is
    // enabled, but loop spawning has not been run. If that changes, those
    // passes should be added here, or before the conditional statement above
    // as appropriate.
  }

  // We always run the runtime intrinsics lowering pass because there may be
  // memory allocation intrinsics that must always be lowered. However, we don't
  // run this during the prelink phase because it should be run during full
  // link time.
  if (!isLTOPreLink(Phase))
...
PassBuilder::buildPerModuleDefaultPipeline(OptimizationLevel Level,
  ...
  // We should run the post-tapir pipeline even if tapir lowering was not
  // performed. Not all passes will necessarily be run though.
...
ModulePassManager PassBuilder::buildThinLTODefaultPipeline(
  ...
  MPM.addPass(
      buildKitsunePreTapirPipeline(Level, ThinOrFullLTOPhase::ThinLTOPostLink));
...
PassBuilder::buildLTODefaultPipeline(OptimizationLevel Level,
  ...
  if (Level == OptimizationLevel::O0) {
    ...
    // Kitsune's runtime intrinsic lowering pass must always run, even if
    // lowering was not performed.
    MPM.addPass(LowerKitsuneRuntimeIntrinsicsPass());
  ...
  // those that must only be run if tapir lowering has been enabled.
  MPM.addPass(
      buildKitsunePreTapirPipeline(Level, ThinOrFullLTOPhase::FullLTOPostLink));
