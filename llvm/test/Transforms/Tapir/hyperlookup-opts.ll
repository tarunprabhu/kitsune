...
;
; RUN: opt < %s -enable-new-pm=0 -tti -tbaa -loop-stripmine -stripmine-loops -indvars -licm -loop-vectorize -instcombine -simplifycfg -S -o - | FileCheck %s
; RUN: opt < %s -aa-pipeline=tbaa,basic-aa -passes='loop-stripmine,loop-mssa(licm),loop-vectorize,instcombine,simplifycfg' -stripmine-loops -S -o - | FileCheck %s
