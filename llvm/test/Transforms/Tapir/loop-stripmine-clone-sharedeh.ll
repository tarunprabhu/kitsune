; RUN: opt < %s -enable-new-pm=0 -loop-stripmine -stripmine-loops -require-parallel-epilog -S | FileCheck %s --check-prefixes=CHECK,CHECK-OLD
; RUN: opt < %s -passes='loop-stripmine' -stripmine-loops -require-parallel-epilog -S | FileCheck %s --check-prefixes=CHECK,CHECK-NEW
