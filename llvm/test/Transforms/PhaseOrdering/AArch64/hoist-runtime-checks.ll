...
; CHECK-NEXT:    [[SUM0:%.*]] = phi i32 [ 0, %[[ENTRY]] ], [ [[ADD:%.*]], %[[IF_END:.*]] ]
; CHECK-NEXT:    ret i32 [[SUM0]]
; CHECK:       [[FOR_BODY:.*]]:
; CHECK-NEXT:    [[IV:%.*]] = phi i64 [ 0, %[[FOR_BODY_PREHEADER]] ], [ [[IV_NEXT:%.*]], %[[IF_END]] ]
; CHECK-NEXT:    [[SUM07:%.*]] = phi i32 [ 0, %[[FOR_BODY_PREHEADER]] ], [ [[ADD]], %[[IF_END]] ]
; CHECK-NEXT:    br i1 [[DOTNOT_NOT]], label %[[IF_END:.*]], label %[[IF_THEN:.*]]
; CHECK:       [[IF_THEN]]:
; CHECK-NEXT:    tail call void @llvm.trap()
; CHECK-NEXT:    unreachable
; CHECK:       [[IF_END]]:
; CHECK-NEXT:    [[ARRAYIDX:%.*]] = getelementptr inbounds i32, ptr [[ARRAY]], i64 [[IV]]
; CHECK-NEXT:    [[TMP2:%.*]] = load i32, ptr [[ARRAYIDX]]
; CHECK-NEXT:    [[ADD]] = add nsw i32 [[TMP2]], [[SUM07]]
; CHECK-NEXT:    [[IV_NEXT]] = add nuw nsw i64 [[IV]], 1
; CHECK-NEXT:    [[EXITCOND_NOT:%.*]] = icmp eq i64 [[IV_NEXT]], [[TMP0]]
...
; CHECK-NEXT:    br label %for.body
; CHECK:       [[FOR_COND_CLEANUP]]:
; CHECK-NEXT:    [[RET_0_LCSSA1:%.*]] = phi i32 [ 0, %entry ], [ [[ADD]], %[[IF_END:.*]] ]
; CHECK-NEXT:    ret i32 [[RET_0_LCSSA1]]
; CHECK:       [[FOR_BODY:.*]]:
; CHECK-NEXT:    [[I_07:%.*]] = phi i64 [ [[INC:%.*]], %[[SPAN_CHECKED_ACCESS:.*]] ], [ 0, %[[FOR_BODY_PREHEADER]] ]
; CHECK-NEXT:    [[RET_06:%.*]] = phi i32 [ [[ADD1:%.*]], %[[SPAN_CHECKED_ACCESS]] ], [ 0, %[[FOR_BODY_PREHEADER]] ]
; CHECK-NEXT:    br i1 [[DOTNOT_NOT]], label %[[SPAN_CHECKED_ACCESS]], label %[[COND_FALSE_I:.*]], !prof [[PROF0:!.*]]
; CHECK:       [[COND_FALSE_I]]:
; CHECK-NEXT:    tail call void @llvm.trap()
; CHECK-NEXT:    unreachable
; CHECK:       [[SPAN_CHECKED_ACCESS]]:
...
;.
; CHECK: [[PROF0]] = !{!"branch_weights", !"expected", i32 2000, i32 1}
