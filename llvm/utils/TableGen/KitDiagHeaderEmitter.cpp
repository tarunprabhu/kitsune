//===- KitDiagHeaderEmitter.cpp - Header for Kitsune-specific diagnostics -===//
//
...
#define DEBUG_TYPE "kit-diags-header"

using namespace llvm;

namespace {

class KitDiagHeaderEmitter {
private:
...
public:
  KitDiagHeaderEmitter(const RecordKeeper &records);
...
void KitDiagHeaderEmitter::emitDiags(raw_ostream &os) {
  ...
  for (StringRef klass : classes) {
    ...
    for (const Record *diag : records.getAllDerivedDefinitions(klass)) {
      StringRef name = diag->getName();
      StringRef msg = diag->getValueAsString("Msg");
      os << macro << "(" << name << ", \"" << msg << "\")\n";
    }
...
void KitDiagHeaderEmitter::emitDiagEnums(raw_ostream &os) {
...
void KitDiagHeaderEmitter::run(raw_ostream &os) {
  emitDiags(os);
  line(os);
  emitDiagEnums(os);
}

KitDiagHeaderEmitter::KitDiagHeaderEmitter(const RecordKeeper &records)
    : records(records) {
  for (const Record *diag : records.getAllDerivedDefinitions("Diag")) {
    const Record *sev = diag->getValueAsDef("Sev");
...
static TableGen::Emitter::OptClass<KitDiagHeaderEmitter>
    X("gen-kit-diag-header",
      "Generate header for Kitsune-specific diagnostics");
