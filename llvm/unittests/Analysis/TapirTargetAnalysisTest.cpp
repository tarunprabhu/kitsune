...
!1 = !{!"llvm.loop.unroll.disable"}
  ...
  kitOpts.setTapirTarget(TapirTargetID::Serial);
  ...
  EXPECT_EQ(*tgi.getIDIfExists(), TapirTargetID::Serial);
  EXPECT_EQ(tgi.getID(), TapirTargetID::Serial);
...
TEST(TapirTargetAnalysisTest, noHints) {
  ...
  // updated.
  std::vector<TapirTargetID> expected = {TapirTargetID::Serial};
...
!5 = !{!"tapir.loop.target", i32 2}
  ...
  // targets or their numerical values are changed, this will need to be
  // updated.
  std::vector<TapirTargetID> expected = {TapirTargetID::Cuda,
                                         TapirTargetID::Hip};
  EXPECT_EQ(tgi.getRequiredTTs(*f), expected);
  EXPECT_EQ(tgi.getRequiredTTs(*m), expected);
}
...
!5 = !{!"tapir.loop.target", i32 3}
  ...
  // targets or their numerical values are changed, this will need to be
  // updated.
  std::vector<TapirTargetID> expectedF = {TapirTargetID::Cuda};
  std::vector<TapirTargetID> expectedG = {TapirTargetID::Hip};
  std::vector<TapirTargetID> expected = {TapirTargetID::Cuda,
                                         TapirTargetID::Hip};
