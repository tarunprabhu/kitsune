...
/// LLVM bitcode. Return the resulting LLVM module, or an error if the
/// initializer of the global could not be parsed.
Expected<std::unique_ptr<Module>> parseEmbBCGlobal(const GlobalVariable &g);
...
/// doing so, it will be returned.
Expected<EmbModulesMapTy> getEmbModules(const Module &m);
