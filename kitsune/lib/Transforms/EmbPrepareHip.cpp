//===- EmbPrepareHip.cpp - Prepare an embedded hip module -----------------===//
//
...
//===----------------------------------------------------------------------===//
//
// Prepare an embedded module, generated by the hip tapir target for (eventual)
// code generation.
//
//===----------------------------------------------------------------------===//
...
public:
  EmbPrepareHip(const TTOptions &tto,
                const detail::EmbPrepareOptions &prepOpts);
...
EmbPrepareHip::EmbPrepareHip(const TTOptions &tto,
                             const detail::EmbPrepareOptions &prepOpts)
...
/// This is far from ideal and goes against "the LLVM way". However, any other
/// approach would probably require a custom cloning utility that transforms
/// types when outlining from the tapir loop to the kernel function. That is
/// likely to be a considerable amount of work. For now, this "works", so we'll
/// stick with it, but it would be nice if we could avoid having to do such
/// things.
bool EmbPrepareHip::fixKernelArgumentAddrSpace(Module &devM) {
  bool changed = false;
  ...
  // pointers. However, the users of these arguments do not expect a pointer in
  // a non-default address space. In some cases, mutating the type of the
  ...
  for (Function &f : devM.functions()) {
    ...
    // We need to create an instruction that casts away the address space in the
    // function arguments that are pointers (the arguments have not been mutated
    // yet). The cast instruction should be added to the entry block of the
    // function after the last alloca.
    ...
    for (Argument &arg : f.args()) {
      if (auto *argTy = dyn_cast<PointerType>(arg.getType())) {
        ...
        if (argAS != AMDGPUAS::GLOBAL_ADDRESS) {
          ...
          // Therefore, mutate the type of the argument first.
          arg.mutateType(ptrASTy);

          // The cast can now be created since it will be valid.
          Value *cst = builder.CreateAddrSpaceCast(&arg, argTy);
      ...
      // all the arguments. We must add the argument type here because it could
      // have changed in the truly appalling code above.
...
/// Fix the address space of all allocas in the device module. These must be in
/// the alloca address space. This works by creating new alloca instructions in
/// the alloca address space, then casting away the address space from the
/// alloca and replacing all uses of the old alloca with this cast.
bool EmbPrepareHip::fixAllocaAddrSpace(Module &devM) {
  bool changed = false;
  LLVMContext &ctx = devM.getContext();
  ...
  for (Function &f : devM.functions()) {
    ...
    for (AllocaInst *ai : toFix) {
      // The new alloca is created just before the alloca that it is replacing.
...
/// attributes that are only relevant if they are run on the CPU. The attributes
/// on the kernel function will have been set by the tapir targets.
bool EmbPrepareHip::fixDeviceFuncAttrs(Module &devM) {
  bool changed = false;
  for (Function &f : devM.functions()) {
    if (hasDeviceAttr(f)) {
...
bool EmbPrepareHip::fixCallingConventions(Module &devM) {
  bool changed = false;

  // Set the calling convention to fast on the device functions because that is
  // what hipcc does.
  for (Function &f : devM.functions()) {
    if (hasDeviceAttr(f)) {
...
bool EmbPrepareHip::run(Module &devM) {
  bool changed = false;
