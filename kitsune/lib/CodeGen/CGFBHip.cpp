//===- CGFBHip.cpp - Generate embedded device code for AMD GPU's ----------===//
//
...
private:
  void embedDeviceCode(ToolOutputFile &mcFile, Module &hostM) {
    ...
    if (std::error_code ec = bufOrErr.getError())
      report_internal_error("error loading amd device code: ", ec);

    Expected<EmbDeviceCode::Id> id = EmbDeviceCode::getIdFor(tto.getHipArch());
    if (not id)
    ...
    payload->setSection(KITSUNE_HIP_CODE_SECTION);
  ...
  std::unique_ptr<ToolOutputFile> genMachineCode(Module &km) {
    ...
    if (ec)
      report_internal_error("could not create amdgpu object file: ", ec);
    ...
    if (tm->addPassesToEmitFile(passMgr, objFile->os(),
                                ...
                                /*DisableVerify=*/false))
      report_internal_error("error generating amd object code");
    passMgr.run(km);
