...
template <typename T> void random_gpu_fill(mobile_ptr<T> data, size_t N) {
  T base_value = rand() / (T)RAND_MAX;
...
int main(int argc, char *argv[]) {
  ...
  mobile_ptr<float> A(ARRAY_SIZE);
  mobile_ptr<float> B(ARRAY_SIZE);
  mobile_ptr<float> C(ARRAY_SIZE);
  cout << "  done.\n\n";
  ...
  if (error_count) {
    ...
    cout << "  pass (answers match).\n\n"
         ...
         << ARRAY_SIZE / elapsed_time << " elements/sec.)\n"
         << "----\n\n";
  }
