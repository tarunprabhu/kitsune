...
template <typename T> using View = Kokkos::DualView<T *>;
...
void cpy(View<float> &dst, View<float> &src, int N) {
  ...
  Kokkos::parallel_for(
      "copy", N,
      KOKKOS_LAMBDA(const int &i) { dst.d_view(i) = src.d_view(i); });
  Kokkos::fence();
  dst.modify_device();
}
...
void initialize_variables(int nelr, View<float> &variables,
                          View<float> &ff_variable) {
  variables.sync_device();
  ff_variable.sync_device();
  Kokkos::parallel_for(
      "initialize_variables", nelr, KOKKOS_LAMBDA(const int &i) {
...
void compute_step_factor(int nelr, View<float> &__restrict variables,
                         View<float> &areas,
  ...
  Kokkos::parallel_for(
      ...
      KOKKOS_LAMBDA(const int &blk) {
        ...
        for (unsigned int i = b_start; i < b_end; i++) {
          ...
          step_factors.d_view(i) =
              float(0.5f) /
              (sqrtf(areas.d_view(i)) * (sqrtf(speed_sqd) + speed_of_sound));
        }
      });
        ...
        for (int i = b_start; i < b_end; ++i) {
          ...
          for (int j = 0; j < NNB; j++) {
            ...
            if (nb >= 0) { // a legitimate neighbor
              ...
              density_energy_nb =
                  variables.d_view(nb + VAR_DENSITY_ENERGY * nelr);
...
void time_step(int j, int nelr, View<float> &old_variables,
               View<float> &variables, View<float> &step_factors,
               View<float> &fluxes) {
...
int main(int argc, char **argv) {
  ...
  Kokkos::initialize(argc, argv);
  {
    ...
    compute_flux_contribution(
        ff_variable.h_view(VAR_DENSITY), ff_momentum,
    ...
    for (int i = 0; i < iterations; i++) {
      ...
      rk_times[i] = chrono::duration<double>(rk_end - rk_start).count();
      rk_total += rk_times[i];
    }
    ...
    // update time to include saving final result (UVM impacts on I/O?). 
