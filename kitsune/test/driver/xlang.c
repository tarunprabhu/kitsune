...
//
// RUN: not %kitcc -x cuda -ftapir=serial -O1 -nocudainc -nocudalib %s 2>&1 \
// RUN:     | FileCheck %s -check-prefix CUDA
// RUN: not %kitcc -x hip -ftapir=serial -O1 -nogpuinc -nogpulib %s 2>&1 \
// RUN:     | FileCheck %s -check-prefix HIP
// RUN: not %kitcc -x objective-c -ftapir=serial -O1 %s 2>&1 \
// RUN:     | FileCheck %s -check-prefix OBJC
// RUN: not %kitcc -x cl -ftapir=serial -O1 %s 2>&1 \
