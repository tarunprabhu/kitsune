; Check that the correct diagnostic is emitted when a tapir loop does not have
; an associated sync instruction.
;
; RUN: not opt --tapir=nolo -passes='kit-verify-prelower' %s 2>&1 \
; RUN:     -disable-output \
; RUN:     | FileCheck %s
;
; CHECK: tapir loop does not have a unique sync instruction
