...
several separate checks:
  ...
  3. A test of the behavior of one or more of the transformation passes when
     operating on LLVM-IR generated by the frontend when the command line
     option was passed to the frontend.
...
directories include:
  ...
  - `assembler/`: See `bitcode/`

  - `bitcode/`: Checks that Kitsune-specific attributes and other constructs
    are parsed/unparsed from/to LLVM bitcode are handled correctly.
    `assembler/` may contain similar tests for human-readable LLVM assembly.
  ...
  - `verifier/`: This intended to check that the verifier works correctly with
    ...
    since the additions are Kitsune-specific. The only reason to have these
    tests live here instead of `llvm/test/Verifier` is to consolidate as much
    of Kitsune's code and tests in one directory as possible. However, if it
    makes more logical sense to add a test to the core LLVM tests in
    `llvm/test`, it is ok to do so.
