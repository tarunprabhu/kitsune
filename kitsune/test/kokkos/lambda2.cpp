...
// CHECK-SAME: i32{{.*}} %[[N:[^)]+]]
// CHECK-NEXT: [[ENTRY:.+]]:
// CHECK-NEXT: %[[SYNCREG1:.+]] = {{.+}}call token @llvm.syncregion.start
// CHECK-NEXT: %[[SYNCREG2:.+]] = {{.+}}call token @llvm.syncregion.start
// CHECK: [[LOOP1:^.+]]:
// CHECK-NEXT: %[[IV1:.+]] = phi i32 [ %[[NEXT1:.+]], %[[LATCH1:.+]] ], [ 0, %[[ENTRY]] ]
// CHECK-NEXT: detach within %[[SYNCREG1]], label %[[BODY1:.+]], label %[[LATCH1]]
// CHECK: [[BODY1]]:
// CHECK-NEXT: call{{.+}} @printf
// CHECK-NEXT: reattach within %[[SYNCREG1]], label %[[LATCH1]]
// CHECK: [[LATCH1]]:
// CHECK-NEXT: %[[NEXT1]] = add {{.+}} %[[IV1]], 1
// CHECK-NEXT: %[[COND1:.+]] = icmp eq i32 %[[NEXT1]], %[[N]]
// CHECK-NEXT: br i1 %[[COND1]], label %[[SYNC1:.+]], label %[[LOOP1]]
// CHECK: [[SYNC1]]:
// CHECK-NEXT: sync within %[[SYNCREG1]]
// CHECK: [[LOOPPH2:^.+]]:
// CHECK: [[LOOP2:^.+]]:
// CHECK-NEXT: %[[IV2:.+]] = phi i32 [ %[[NEXT2:.+]], %[[LATCH2:.+]] ], [ 0, %[[LOOPPH2]] ]
// CHECK-NEXT: detach within %[[SYNCREG2]], label %[[BODY2:.+]], label %[[LATCH2]]
// CHECK: [[BODY2]]:
// CHECK-NEXT: call{{.+}} @printf
// CHECK-NEXT: reattach within %[[SYNCREG2]], label %[[LATCH2]]
// CHECK: [[LATCH2]]:
