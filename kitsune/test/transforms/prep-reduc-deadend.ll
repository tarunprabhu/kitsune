; Check that a tapir reduction loop containing an exit to a dead-end block is
; lowered correctly. While we don't allow early exits in parallel loops in the
; frontend, branches to dead-end blocks can occur as a result of certain
; optimization passes.
;
; RUN: opt --tapir=openmp --passes=kit-prepare -S %s | FileCheck %s
;
; CHECK-LABEL: @f
; CHECK-SAME: i64 %[[N:[^,]+]]
; CHECK-SAME: i64 %[[B:[^)]+]]
...
; CHECK-EMPTY:
; CHECK-NEXT: [[PH_O]]:
; CHECK-NEXT: %[[NREDS:.+]] = call i64 @llvm.kit.cpu.num.threads(i32 512)
...
; CHECK-EMPTY:
; CHECK-NEXT: [[PH_I]]:
; CHECK-NEXT: %[[LOCAL:.+]] = tail call noalias ptr @llvm.kit.cpu.malloc(i32 512, i64 8)
...
; CHECK-NEXT: atomicrmw umax ptr %[[RESULT]], i64 %[[PARTIAL]] monotonic
; CHECK-NEXT: call void @llvm.kit.cpu.free(i32 512, ptr %[[LOCAL]])
...
declare void @max(ptr %res, i64 %v)
...
for.i.body:
  call void(i32, i32, ptr, i32, i64, i64, ptr, ...) @llvm.kit.reduce.0(i32 512, i32 26, ptr %result, i32 8, i64 %i, i64 0, ptr @max)
  %chk = icmp eq i64 %i, %b
