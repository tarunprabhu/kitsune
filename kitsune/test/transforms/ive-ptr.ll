; Only integer and floating point secondary inductions are currently supported.
;
; RUN: not opt -passes='kit-ive' -S %s 2>&1 | FileCheck %s
;
; CHECK: secondary induction variable has unsupported type 'ptr'

declare ptr @next(ptr)
