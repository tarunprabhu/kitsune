; The incoming value from the loop latch of a secondary induction variable must
; be a binary operator.
;
; RUN: not opt -passes='kit-ive' -S %s 2>&1 | FileCheck %s
;
; CHECK: secondary induction variable has unsupported incoming value

define i64 @inc(i64 %n) {
  %1 = add i64 %n, 1
  ret i64 %1
}
