...
// a success code
// RUN: %kitxx -fsyntax-only -ftapir=realm %s
// RUN: %kitxx -### -ftapir=realm %s

// The option value is case sensitive.
// RUN: not %kitxx -fsyntax-only -ftapir=Realm %s 2>&1 | FileCheck %s -check-prefix=CHECK-BAD-TARGET
// RUN: %kitxx -### -ftapir=Realm %s 2>&1 | FileCheck %s -check-prefix=CHECK-BAD-TARGET
