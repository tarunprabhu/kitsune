; Check that the finalize kernel metadata pass updates the initializer of the
...
; RUN:     | FileCheck %s
;
; CHECK: @{{.+}} = private unnamed_addr constant {{.+}} { {{.+}} }, !kitsune.kernel.md ![[KERNEL_MD:[0-9]+]]
; CHECK: ![[KERNEL_MD]] = !{[{{[0-9]+}} x i8] c"{{.+}}"}
