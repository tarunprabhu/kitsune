...
!
! RUN: %kitfc -### -ftapir=opencilk -O1 %s 2>&1 \
...
!
! RUN: %kitfc -### -ftapir=opencilk -O1 --config-kitsune-dir=%S %s 2>&1 \
...
!
! RUN: %kitfc -### -ftapir=opencilk -O1 --config-kitsune-dir=%S/input %s 2>&1 \
...
! CHECK-CUSTOM: Configuration file: {{.*}}/input/opencilk.cfg
! CHECK-CUSTOM: "-fc1"
