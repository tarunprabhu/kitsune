...
;
; RUN: opt --tapir=opencilk --passes=kit-reductions -S %s \
; RUN:     | FileCheck %s
;
; CHECK-LABEL: @f
; CHECK-SAME: i64 %[[N:[^)]+]]
...
; CHECK: %[[NREDS:.+]] = call i64 @llvm.kit.reduce.partials.count(i32 8, i64 %[[N]])
...
; CHECK-NEXT: store float [[UNIT:.+]], {{.+}} %[[ADDR_INIT]]
...
define void @sum(ptr %res, float %v) {
  %1 = load float, ptr %res
  %2 = fadd float %1, %v
  store float %2, ptr %res
  ret void
}
...
for.i.body:
  ...
  call void(i32, ptr, i32, float, float, ptr, ...) @llvm.kit.reduce.0(i32 8, ptr %result, i32 4, float %f32, float 0.0, ptr @sum)
