...
// RUN:   %kitxx --tapir=opencilk --target=x86_64-unknown-linux-gnu \
...
//
// RUN: %if x86-registered-target %{ \
// RUN:   %kitxx --tapir=opencilk --target=x86_64-pc-freebsd \
// RUN:       -Xclang --tapir-opencilk-runtime-bc=%S/input/libopencilk-abi.bc \
// RUN:       -c -O2 %s 2>&1 \
// RUN:       | FileCheck --allow-empty -check-prefix SUPPORTED %s \
// RUN: %}
//
// RUN: %if x86-registered-target %{ \
// RUN:   %kitxx --tapir=opencilk --target=x86_64-apple-macosx \
// RUN:       -Xclang --tapir-opencilk-runtime-bc=%S/input/libopencilk-abi.bc \
// RUN:       -c -O2 %s 2>&1 \
// RUN:       | FileCheck --allow-empty -check-prefix SUPPORTED %s \
// RUN: %}
//
// RUN: %if x86-registered-target %{ \
// RUN:   not %kitxx --tapir=opencilk --target=x86_64-pc-openbsd \
// RUN:       -Xclang --tapir-opencilk-runtime-bc=%S/input/libopencilk-abi.bc \
// RUN:       -c -O2 %s 2>&1 \
...
// RUN:   %kitxx --tapir=opencilk --target=aarch64-unknown-linux-gnu %s \
// RUN:       -Xclang --tapir-opencilk-runtime-bc=%S/input/libopencilk-abi.bc \
// RUN:       -c -O2 %s 2>&1 \
// RUN:       | FileCheck --allow-empty -check-prefix SUPPORTED %s \
// RUN: %}
...
// RUN:   not %kitxx --tapir=opencilk --target=sparc-pc-linux-gnu %s \
// RUN:       -Xclang --tapir-opencilk-runtime-bc=%S/input/libopencilk-abi.bc \
// RUN:       -c -O2 %s 2>&1 \
