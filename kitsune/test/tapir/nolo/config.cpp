...
//
// RUN: %kitxx -### --tapir=nolo -O1 %s 2>&1 \
// RUN:     | FileCheck %s -check-prefix=DEFAULT
//
// DEFAULT: Configuration file: {{.*}}/nolo.cfg
//
// -----------------------------------------------------------------------------
// Check that providing a custom config directory without a target-specific
// configuration file is ok.
//
// RUN: %kitxx -### --tapir=nolo -O1 --config-kitsune-dir=%S/ %s 2>&1 \
// RUN:     | FileCheck %s -check-prefix=CUSTOM-NOEXIST
//
// COM: %kitxx -### --tapir=nolo -O1 --config-user-dir=%S/ %s 2>&1 \
// COM:     | FileCheck %s -check-prefix=CUSTOM-NOEXIST
//
// CUSTOM-NOEXIST-NOT: Configuration file: {{.*}}/nolo.cfg
//
// -----------------------------------------------------------------------------
...
//
// RUN: %kitxx -### --tapir=nolo -O1 --config-kitsune-dir=%S/input %s 2>&1 \
// RUN:     | FileCheck %s -check-prefix=CUSTOM
//
// RUN: %kitxx -### --tapir=nolo -O1 --config-user-dir=%S/input %s 2>&1 \
// RUN:     | FileCheck %s -check-prefix=CUSTOM
//
