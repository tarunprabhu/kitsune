...
!
! RUN: %kitfc -### --tapir=nolo -O1 --config-system-dir=%S %s 2>&1 \
! RUN:     | FileCheck %s -check-prefix=CUSTOM-NOEXIST
!
! COM: %kitfc -### --tapir=nolo -O1 --config-user-dir=%S %s 2>&1 \
...
!
! RUN: %kitfc -### --tapir=nolo -O1 --config-system-dir=%S/input %s 2>&1 \
! RUN:     | FileCheck %s -check-prefix=CUSTOM
!
! RUN: %kitfc -### --tapir=nolo -O1 --config-user-dir=%S/input %s 2>&1 \
! RUN:     | FileCheck %s -check-prefix=CUSTOM
!
