...
!
! RUN: %kitfc -### --tapir=openmp -O1 --config-system-dir=%S/ %s 2>&1 \
! RUN:     | FileCheck %s -check-prefix=CHECK-CUSTOM-NOEXIST
!
! COM: %kitfc -### --tapir=openmp -O1 --config-user-dir=%S/ %s 2>&1 \
...
!
! RUN: %kitfc -### --tapir=openmp -O1 --config-system-dir=%S/input %s 2>&1 \
! RUN:     | FileCheck %s -check-prefix=CHECK-CUSTOM
!
! RUN: %kitfc -### --tapir=openmp -O1 --config-user-dir=%S/input %s 2>&1 \
! RUN:     | FileCheck %s -check-prefix=CHECK-CUSTOM
!
...
! RUN: %kitfc -### --tapir=openmp -O1 %s 2>&1 \
