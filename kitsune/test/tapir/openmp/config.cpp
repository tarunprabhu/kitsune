...
//
// RUN: %kitxx -### --tapir=openmp -O1 --config-system-dir=%S/ %s 2>&1 \
// RUN:     | FileCheck %s -check-prefix=CUSTOM-NOEXIST
//
// COM: %kitxx -### --tapir=openmp -O1 --config-user-dir=%S/ %s 2>&1 \
// COM:     | FileCheck %s -check-prefix=CUSTOM-NOEXIST
//
// CUSTOM-NOEXIST-NOT: Configuration file: {{.*}}/openmp.cfg
//
// -----------------------------------------------------------------------------
...
//
// RUN: %kitxx -### --tapir=openmp -O1 --config-system-dir=%S/input %s 2>&1 \
// RUN:     | FileCheck %s -check-prefix=CUSTOM
//
// RUN: %kitxx -### --tapir=openmp -O1 --config-user-dir=%S/input %s 2>&1 \
// RUN:     | FileCheck %s -check-prefix=CUSTOM
//
// CUSTOM: Configuration file: {{.*}}/input/openmp.cfg
// CUSTOM: "-cc1"
// CUSTOM-SAME: "-D" "openmp_preprocessor_flag"
// CUSTOM-SAME: "-Wopenmp_compiler_flag"
// CUSTOM-NEXT: "-openmp_linker_flag"
//
// -----------------------------------------------------------------------------
...
// RUN: %kitxx -### --tapir=openmp -O1 %s 2>&1 \
