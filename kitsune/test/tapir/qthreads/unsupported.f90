...
!
! RUN: %if x86-registered-target %{ \
! RUN:   %kitfc --tapir=qthreads --target=x86_64-apple-macosx \
! RUN:       -c -O2 %s 2>&1 \
! RUN:       | FileCheck --allow-empty -check-prefix SUPPORTED %s \
! RUN: %}
!
! RUN: %if x86-registered-target %{ \
! RUN:   not %kitfc --tapir=qthreads --target=x86_64-pc-freebsd \
! RUN:       -c -O2 %s 2>&1 \
! RUN:       | FileCheck --allow-empty -check-prefix PLATFORM %s \
! RUN: %}
!
! RUN: %if x86-registered-target %{ \
! RUN:   not %kitfc --tapir=qthreads --target=x86_64-pc-openbsd \
! RUN:       -c -O2 %s 2>&1 \
! RUN:       | FileCheck --allow-empty -check-prefix PLATFORM %s \
! RUN: %}
!
...
! RUN:   not %kitfc --tapir=qthreads --target=sparc-pc-linux-gnu \
! RUN:       -c -O2 %s 2>&1 \
