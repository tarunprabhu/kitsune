...
// RUN: %kitxx -### --tapir=qthreads --config-system-dir=%S/../ %s 2>&1 | FileCheck %s -check-prefix=CHECK-CUSTOM-NOEXIST
...
// RUN: %kitxx -### --tapir=qthreads --config-system-dir=%S %s 2>&1 | FileCheck %s -check-prefix=CHECK-CUSTOM
