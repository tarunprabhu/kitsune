...
//
// RUN: %kitxx -### --tapir=qthreads -O1 --config-system-dir=%S/ %s 2>&1 \
// RUN:     | FileCheck %s -check-prefix=CUSTOM-NOEXIST
//
// COM: %kitxx -### --tapir=qthreads -O1 --config-user-dir=%S/ %s 2>&1 \
// COM:     | FileCheck %s -check-prefix=CUSTOM-NOEXIST
//
// CUSTOM-NOEXIST-NOT: Configuration file: {{.*}}/qthreads.cfg
//
// -----------------------------------------------------------------------------
...
//
// RUN: %kitxx -### --tapir=qthreads -O1 --config-system-dir=%S/input %s 2>&1 \
// RUN:     | FileCheck %s -check-prefix=CUSTOM
//
// RUN: %kitxx -### --tapir=qthreads -O1 --config-user-dir=%S/input %s 2>&1 \
// RUN:     | FileCheck %s -check-prefix=CUSTOM
//
...
// RUN: %kitxx -### --tapir=qthreads -O1 %s 2>&1 \
