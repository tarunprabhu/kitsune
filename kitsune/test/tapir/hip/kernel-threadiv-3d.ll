...
; CHECK-SAME: #{{[0-9]+}}
;
; CHECK: %[[TID_Z:.+]] = {{.*}}call i32 @llvm.kit.gpu.thread.id.z(i32 4)
; CHECK: %[[BID_Z:.+]] = {{.*}}call i32 @llvm.kit.gpu.block.id.z(i32 4)
; CHECK: %[[BSZ_Z:.+]] = {{.*}}call i32 @llvm.kit.gpu.block.size.z(i32 4)
; CHECK: %[[OFF_Z:.+]] = mul i32 %[[BSZ_Z]], %[[BID_Z]]
; CHECK: %[[IVBEG32_Z:.+]] = add i32 %[[OFF_Z]], %[[TID_Z]]
; CHECK: %[[IVBEG_Z:.+]] = zext i32 %[[IVBEG32_Z]] to i64
;
; CHECK: %[[TID_Y:.+]] = {{.*}}call i32 @llvm.kit.gpu.thread.id.y(i32 4)
; CHECK: %[[BID_Y:.+]] = {{.*}}call i32 @llvm.kit.gpu.block.id.y(i32 4)
; CHECK: %[[BSZ_Y:.+]] = {{.*}}call i32 @llvm.kit.gpu.block.size.y(i32 4)
; CHECK: %[[OFF_Y:.+]] = mul i32 %[[BSZ_Y]], %[[BID_Y]]
; CHECK: %[[IVBEG32_Y:.+]] = add i32 %[[OFF_Y]], %[[TID_Y]]
; CHECK: %[[IVBEG_Y:.+]] = zext i32 %[[IVBEG32_Y]] to i64
;
; CHECK: %[[TID_X:.+]] = {{.*}}call i32 @llvm.kit.gpu.thread.id.x(i32 4)
; CHECK: %[[BID_X:.+]] = {{.*}}call i32 @llvm.kit.gpu.block.id.x(i32 4)
; CHECK: %[[BSZ_X:.+]] = {{.*}}call i32 @llvm.kit.gpu.block.size.x(i32 4)
