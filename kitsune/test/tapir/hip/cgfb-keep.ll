...
; RUN: ls -l %t/kithip-*-cgfbkeep.* | FileCheck %s -check-prefix=EXT
; RUN: ls -l %t/kithip-*-cgfbkeep.* | FileCheck %s -check-prefix=COUNT
;
; EXT-DAG: {{[.]amdgpu.o}}
; EXT-DAG: {{[.]amdgpu.so}}
; COUNT-COUNT-2: {{.+}}
...
@.kitsune.emb.fb = constant [0 x i8] zeroinitializer, section ".hip_fatbin", align 4096 #1
