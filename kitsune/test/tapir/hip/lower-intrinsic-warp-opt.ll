; Check the Kitsune's warp index and lane intrinsics get optimized as expected.
;
; RUN: opt -passes='kit-lower-warp-intrinsics,default<O2>' -S %s \
; RUN:     | FileCheck %s
;

; CHECK-LABEL: @id1
; CHECK-NEXT: %[[X:.+]] = tail call i32 @llvm.kit.gpu.thread.id.x(i32 4)
; CHECK-NEXT: %[[WARPSIZE:.+]] = tail call i32 @llvm.kit.gpu.warp.size(i32 4)
; CHECK-NEXT: %[[RESULT:.+]] = udiv i32 %[[X]], %[[WARPSIZE]]
; CHECK-NEXT: ret i32 %[[RESULT]]
define i32 @id1() {
  %1 = call i32 @llvm.kit.gpu.warp.id(i32 4, i32 1)
  ret i32 %1
}

; CHECK-LABEL: @id2
; CHECK-NEXT: %[[BSZX:.+]] = tail call i32 @llvm.kit.gpu.block.size.x(i32 4)
; CHECK-NEXT: %[[TIDY:.+]] = tail call i32 @llvm.kit.gpu.thread.id.y(i32 4)
; CHECK-NEXT: %[[Y:.+]] = mul i32 %[[TIDY]], %[[BSZX]]
; CHECK-NEXT: %[[X:.+]] = tail call i32 @llvm.kit.gpu.thread.id.x(i32 4)
; CHECK-NEXT: %[[OFFXY:.+]] = add i32 %[[X]], %[[Y]]
; CHECK-NEXT: %[[WARPSIZE:.+]] = tail call i32 @llvm.kit.gpu.warp.size(i32 4)
; CHECK-NEXT: %[[RESULT:.+]] = udiv i32 %[[OFFXY]], %[[WARPSIZE]]
; CHECK-NEXT: ret i32 %[[RESULT]]
define i32 @id2() {
  %1 = call i32 @llvm.kit.gpu.warp.id(i32 4, i32 2)
  ret i32 %1
}

; CHECK-LABEL: @id3
; CHECK-NEXT: %[[BSZX:.+]] = tail call i32 @llvm.kit.gpu.block.size.x(i32 4)
; CHECK-NEXT: %[[BSZY:.+]] = tail call i32 @llvm.kit.gpu.block.size.y(i32 4)
; CHECK-NEXT: %[[TIDZ:.+]] = tail call i32 @llvm.kit.gpu.thread.id.z(i32 4)
; CHECK-NEXT: %[[Z:.+]] = mul i32 %[[TIDZ]], %[[BSZY]]
; CHECK-NEXT: %[[TIDY:.+]] = tail call i32 @llvm.kit.gpu.thread.id.y(i32 4)
; CHECK-NEXT: %[[X:.+]] = tail call i32 @llvm.kit.gpu.thread.id.x(i32 4)
; CHECK-NEXT: %[[ADD:.+]] = add i32 %[[TIDY]], %[[Z]]
; CHECK-NEXT: %[[MUL:.+]] = mul i32 %[[ADD]], %[[BSZX]]
; CHECK-NEXT: %[[OFFXYZ:.+]] = add i32 %[[MUL]], %[[X]]
; CHECK-NEXT: %[[WARPSIZE:.+]] = tail call i32 @llvm.kit.gpu.warp.size(i32 4)
; CHECK-NEXT: %[[RESULT:.+]] = udiv i32 %[[OFFXYZ]], %[[WARPSIZE]]
; CHECK-NEXT: ret i32 %[[RESULT]]
define i32 @id3() {
  %1 = call i32 @llvm.kit.gpu.warp.id(i32 4, i32 3)
  ret i32 %1
}

; CHECK-LABEL: @lane1
; CHECK-NEXT: %[[X:.+]] = tail call i32 @llvm.kit.gpu.thread.id.x(i32 4)
; CHECK-NEXT: %[[WARPSIZE:.+]] = tail call i32 @llvm.kit.gpu.warp.size(i32 4)
; CHECK-NEXT: %[[RESULT:.+]] = urem i32 %[[X]], %[[WARPSIZE]]
; CHECK-NEXT: ret i32 %[[RESULT]]
define i32 @lane1() {
  %1 = call i32 @llvm.kit.gpu.warp.lane(i32 4, i32 1)
  ret i32 %1
}

; CHECK-LABEL: @lane2
; CHECK-NEXT: %[[BSZX:.+]] = tail call i32 @llvm.kit.gpu.block.size.x(i32 4)
; CHECK-NEXT: %[[TIDY:.+]] = tail call i32 @llvm.kit.gpu.thread.id.y(i32 4)
; CHECK-NEXT: %[[Y:.+]] = mul i32 %[[TIDY]], %[[BSZX]]
; CHECK-NEXT: %[[X:.+]] = tail call i32 @llvm.kit.gpu.thread.id.x(i32 4)
; CHECK-NEXT: %[[OFFXY:.+]] = add i32 %[[X]], %[[Y]]
; CHECK-NEXT: %[[WARPSIZE:.+]] = tail call i32 @llvm.kit.gpu.warp.size(i32 4)
; CHECK-NEXT: %[[RESULT:.+]] = urem i32 %[[OFFXY]], %[[WARPSIZE]]
; CHECK-NEXT: ret i32 %[[RESULT]]
define i32 @lane2() {
  %1 = call i32 @llvm.kit.gpu.warp.lane(i32 4, i32 2)
  ret i32 %1
}

; CHECK-LABEL: @lane3
; CHECK-NEXT: %[[BSZX:.+]] = tail call i32 @llvm.kit.gpu.block.size.x(i32 4)
; CHECK-NEXT: %[[BSZY:.+]] = tail call i32 @llvm.kit.gpu.block.size.y(i32 4)
; CHECK-NEXT: %[[TIDZ:.+]] = tail call i32 @llvm.kit.gpu.thread.id.z(i32 4)
; CHECK-NEXT: %[[Z:.+]] = mul i32 %[[TIDZ]], %[[BSZY]]
; CHECK-NEXT: %[[TIDY:.+]] = tail call i32 @llvm.kit.gpu.thread.id.y(i32 4)
; CHECK-NEXT: %[[X:.+]] = tail call i32 @llvm.kit.gpu.thread.id.x(i32 4)
; CHECK-NEXT: %[[ADD:.+]] = add i32 %[[TIDY]], %[[Z]]
; CHECK-NEXT: %[[MUL:.+]] = mul i32 %[[ADD]], %[[BSZX]]
; CHECK-NEXT: %[[OFFXYZ:.+]] = add i32 %[[MUL]], %[[X]]
; CHECK-NEXT: %[[WARPSIZE:.+]] = tail call i32 @llvm.kit.gpu.warp.size(i32 4)
; CHECK-NEXT: %[[RESULT:.+]] = urem i32 %[[OFFXYZ]], %[[WARPSIZE]]
; CHECK-NEXT: ret i32 %[[RESULT]]
define i32 @lane3() {
  %1 = call i32 @llvm.kit.gpu.warp.lane(i32 4, i32 3)
  ret i32 %1
}
