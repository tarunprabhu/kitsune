...
; CHECK: %[[NREDS:.+]] = call i64 @llvm.kit.gpu.num.compute.units(i32 4)
; CHECK-NEXT: %[[BYTES:.+]] = mul {{.+}} 4, %[[NREDS]]
; CHECK-NEXT: %[[REDS:.+]] = call {{.+}} @llvm.kit.mobile.alloc(i32 4, i64 %[[BYTES]])
...
; CHECK-NEXT: call void @llvm.kit.mobile.free(i32 4, ptr {{.+}} %[[REDS]])
