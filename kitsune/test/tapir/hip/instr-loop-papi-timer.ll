...
; on an AMD GPU. But if other instrumentation is also enabled, that should be
; added.
;
; RUN: opt -passes="kit-instrument" -S %s \
; RUN:     --kit-instr-unit=loop  --kit-instr=papi,timer 2>&1 \
; RUN:     | FileCheck %s
;
; CHECK: cannot add PAPI instrumentation to loop with tapir target 'hip'
; CHECK-NEXT: from loop 'skip-papi'
;
; CHECK: @[[NAME:.+]] = private{{.*}} constant [10 x i8] c"skip-papi\00"
;
; CHECK-LABEL: @f
; CHECK-NOT: __kit_papi_start
; CHECK-NOT: __kit_papi_stop
; CHECK: %[[EPOCH:.+]] = call ptr @__kittimer_start(ptr @[[NAME]], i64 0)
