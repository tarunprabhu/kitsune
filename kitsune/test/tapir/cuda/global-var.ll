...
;
; CHECK: define {{.+}} @f
; CHECK: %[[PTR1:.+]] = tail call {{.+}} @llvm.kit.symbol.device.ptr(i32 2, ptr nonnull @[[FB]], ptr nonnull @[[VARNAME]])
; CHECK: call {{.+}} @llvm.kit.symbol.memcpy.htod(i32 2, ptr %[[PTR1]], ptr nonnull @[[HOSTVAR]], i64 4)
; CHECK: %[[TS:.+]] = call {{.+}} @llvm.kit.async.launch.kernel(i32 2, ptr nonnull @[[FB]],
; CHECK: %[[PTR2:.+]] = call {{.+}} @llvm.kit.symbol.device.ptr(i32 2, ptr nonnull @[[FB]], ptr nonnull @[[VARNAME]])
; CHECK: call {{.+}} @llvm.kit.symbol.memcpy.dtoh(i32 2, ptr nonnull @[[HOSTVAR]], ptr %[[PTR2]], i64 4)
; CHECK: ret void
; CHECK-NEXT: }
;
