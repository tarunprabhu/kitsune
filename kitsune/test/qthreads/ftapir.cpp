...
// a success code
// RUN: %kitxx -fsyntax-only -ftapir=qthreads %s
// RUN: %kitxx -### -ftapir=qthreads %s

// The option value is case sensitive.
// RUN: not %kitxx -fsyntax-only -ftapir=Qthreads %s 2>&1 | FileCheck %s -check-prefix=CHECK-BAD-TARGET
// RUN: %kitxx -### -ftapir=Qthreads %s 2>&1 | FileCheck %s -check-prefix=CHECK-BAD-TARGET
