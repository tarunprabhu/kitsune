...
Global variables may be used in a **`forall`** loop. Any value that is not a
local variable declared in the function/method containing the **`forall`** loop
or a parameter to that function/method is considered a global variable.
...
- The globals must be compile-time constants and must be declared as such
  ...
  **`const`**. _[In the future, this restriction may be relaxed to allow
  ...
  within the **`forall`** loop even if they are not declared **`const`** in
  the source code]._
