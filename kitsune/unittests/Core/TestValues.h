//===- TestValues.h - Utilities to get simple "random" values for tests ---===//
//
...
T get(unsigned idx) {
  // FIXME: This is dangerous! There is no guarantee that the values 1 and 2
  // will be valid for any enum that we might want to use here. But for now, we
  // can get away with it.
  static constexpr T pool[] = {(T)1, (T)2};
  return pool[idx % 2];
}
