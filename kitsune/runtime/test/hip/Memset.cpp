...
template <typename T> static unsigned test(unsigned n, T init) {
  ...
  if constexpr (std::is_same_v<T, bool>)
    __kithip_memset_bool((T*)buf, n, init);
  else if constexpr (std::is_same_v<T, int8_t> || std::is_same_v<T, uint8_t>)
    __kithip_memset_i8((T*)buf, n, init);
  else if constexpr (std::is_same_v<T, int16_t> || std::is_same_v<T, uint16_t>)
    __kithip_memset_i16((T*)buf, n, init);
  else if constexpr (std::is_same_v<T, int32_t> || std::is_same_v<T, uint32_t>)
    __kithip_memset_i32((T*)buf, n, init);
  else if constexpr (std::is_same_v<T, int64_t> || std::is_same_v<T, uint64_t>)
    __kithip_memset_i64((T*)buf, n, init);
  else if constexpr (std::is_same_v<T, float>)
    __kithip_memset_float((T*)buf, n, init);
  else if constexpr (std::is_same_v<T, double>)
    __kithip_memset_double((T*)buf, n, init);
  else
